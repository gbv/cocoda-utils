...
function languageMapContent(item, prop, _options = {}) {
  ...
  if (object) {
    language = getLanguage(object, language, _options)
    if (language) {
...
function prefLabel(item, _options = {}) {
  ...
  let content = languageMapContent(item, "prefLabel", _options)
  if (content) {
    return content
  }
...
function definition(item, _options = {}) {
  let content = languageMapContent(item, "definition", _options)
  if (!content) {
    return []
  }
