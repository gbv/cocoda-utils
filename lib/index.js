...
class CocodaUtils {
   ...
   *
   */
  constructor({ languages, delay, licenseBadges } = {}) {
 ...
 */
   ...
   * @param {*} language
   */
  getLanguage(languageMap, language) {
    if (!languageMap) {
      return null
    }
    let languages = [language].concat(this.languages)
  ...
  lmContent(item, prop, language) {
    ...
    if (object) {
      language = this.getLanguage(object, language)
      if (language) {
   ...
   */
  prefLabel(item, language, fallbackToUri = true) {
    let content = this.lmContent(item, "prefLabel", language)
    if (content) {
      return content
    }
   ...
   * @param {*} language
   */
  definition(item, language) {
    let content = this.lmContent(item, "definition", language)
    if (!content) {
      return []
    }
