...
class CocodaUtils {
   ...
   */
  constructor({ store, storePath, languages } = {}) {
    this.store = store
 ...
 */
   ...
   */
  getLanguage(languageMap, language, { languages } = {}) {
    languages = languages || _.get(this.store, this.storePath) || this.languages
    if (!languageMap) {
      return null
    }
  ...
  definition(item, options = {}) {
    let content = this.lmContent(item, "definition", options)
    if (!content) {
      return []
    }
